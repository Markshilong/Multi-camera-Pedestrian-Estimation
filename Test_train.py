...
def main(hp, num_epochs, resume, name):
    ...
    train_dataloader = DataLoader(
        dataset_train, batch_size=8, num_workers=8, shuffle=True
    )
    val_dataloader = DataLoader(
        dataset_val, batch_size=1, num_workers=8, shuffle=False
    )
...
def validation(valid_loader, model, criterion, logger, step):
    ...
    for idx, data in enumerate(tqdm(valid_loader, desc="validation")):
        ...
        if idx == 0:
            logger.log_images(inputs.cpu()[0], labels.cpu()[0], outputs.cpu()[0], step)
